...
impl<A: Arch, F: FrameAllocator> PageMapper<A, F> {
    ...
    pub unsafe fn create(table_kind: TableKind, mut allocator: F) -> Option<Self> {
        ...
        Some(unsafe { Self::new(table_kind, table_addr, allocator) })
    }

    pub unsafe fn current(table_kind: TableKind, allocator: F) -> Self {
        let table_addr = unsafe { A::table(table_kind) };
        // SAFETY: We have a valid table_addr from A::table() and table_kind from parameter
        unsafe { Self::new(table_kind, table_addr, allocator) }
    }
    ...
    pub unsafe fn make_current(&self) {
        unsafe { A::set_table(self.table_kind, self.table_addr) };
    }
    ...
    pub unsafe fn remap_with_full(
        ...
        self.visit(virt, |p1, i| {
            let old_entry = unsafe { p1.entry(i)? };
            ...
            unsafe { p1.set_entry(i, new_entry) };
    ...
    pub unsafe fn remap_with(
        ...
        unsafe { self.remap_with_full(virt, |same_phys, old_flags| {
    ...
    pub unsafe fn map(
        ...
        unsafe { self.map_phys(virt, phys, flags) }
    }
    ...
    pub unsafe fn map_phys(
        ...
        loop {
            let i = unsafe { table.index_of(virt)? };
            if table.level() == 0 {
                unsafe { table.set_entry(i, entry) };
                return Some(PageFlush::new(virt));
            } else {
                let next_opt = unsafe { table.next(i) };
                let next = match next_opt {
                    ...
                    None => {
                        ...
                        unsafe { table.set_entry(i, PageEntry::new(next_phys.data(), flags)) };
                        unsafe { table.next(i)? }
                    }
                };
                table = next;
    ...
    pub unsafe fn map_linearly(
        ...
        let virt = unsafe { A::phys_to_virt(phys) };
        unsafe { self.map_phys(virt, phys, flags).map(|flush| (virt, flush)) }
    }
    fn visit<T>(
        &self,
        virt: VirtualAddress,
        f: impl FnOnce(&mut PageTable<A>, usize) -> Option<T>,
    ) -> Option<T> {
        let mut table = self.table();
        unsafe {
            loop {
    ...
    pub fn translate(&self, virt: VirtualAddress) -> Option<(PhysicalAddress, PageFlags<A>)> {
        let entry = match self.visit(virt, |p1, i| unsafe { p1.entry(i) }) {
            Some(entry) => entry,
            _ => return None,
        };
    ...
    pub unsafe fn unmap(
        ...
        let (old, _, flush) = unsafe { self.unmap_phys(virt, unmap_parents)? };
        unsafe { self.allocator.free_one(old) };
        Some(flush)
    }
...
unsafe fn unmap_phys_inner<A: Arch>(
    ...
    if table.level() == 0 {
        let entry_opt = unsafe { table.entry(i) };
        unsafe { table.set_entry(i, PageEntry::new(0, 0)) };
        let entry = entry_opt?;

        Some((entry.address().ok()?, entry.flags()))
    } else {
        let mut subtable = unsafe { table.next(i) }?;
        ...
        if unmap_parents {
            ...
            let is_still_populated = (0..A::PAGE_ENTRIES)
                .map(|j| unsafe { subtable.entry(j).expect("must be within bounds") })
                .any(|e| e.present());
